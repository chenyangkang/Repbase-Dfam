...
def GBDT1_training(X_train, X_test, y_train_binary, weights, y_test_binary=None):
    ...
    GBDT = RandomForestClassifier(n_estimators=2000, class_weight='balanced', max_depth=5)
...
def get_and_print_GBDT1_metrics(GBDT, y_train_binary, y_test_binary, GBDT_pred_train, GBDT_pred_test, X_train):
    scaler = MinMaxScaler().fit(GBDT_pred_train.reshape(-1,1))
    GBDT_pred_train = scaler.transform(GBDT_pred_train.reshape(-1,1))
    GBDT_pred_test = scaler.transform(GBDT_pred_test.reshape(-1,1))
    print(' ')
